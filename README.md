# Base EFI for Intel X99 Broadwell-E

This is a base EFI which I have created for a Gigabyte GA-X99P-SLI.

Should be working with any Broadwell-E and Haswell-E CPU.
...
:----|:----
SSDT-RTC0-RANGE-HEDT.aml|Fixes RTC clock issues.
SSDT-PLUG-DRTNIA.aml|Enables native CPU Power Management. 
SSDT-EC-USBX-DESKTOP.aml|Fixes both the embedded controller and USB power.
SSDT-UNC.aml|Fixes Uncore Bridges. This SSDT is required for all X99 and many X79 boards, it specifically disables unused devices in ACPI ensuing IOPCIFamily doesn't kernel panic.


## Optional ACPI
.aml-file|Description
:----|:----
SSDT-HPET.aml|Fix for IRQ Conflicts.
SSDT-DTGP.aml|Thunderbolt 3 Support.
SSDT-TBOLT3.aml|Thunderbolt 3 Support. You have to check the PCI-path of your Thunderbolt device. On Gigabyte GC-X99P-SLI it is RP01.
...
USBPorts.kext|You need a USB-port mapping that stays within the 15 port limit. Total number of HSxx and SSxx ports count. Otherwise macOS will not boot. This is just a sample of the Gigabyte GA-X99P-SLI which stays within the 15 ports limit and is a good initial setup. Many ports will work with other boards, but you need to customize this for your board if you want all the ports working correctly. Hint: This Kext is for MacPro6,1 SMBIOS. If you want to use is with other SMBIOS like iMacPro1,1 you have to search and replace the SMBIOS in the info.plist of the Kext.
XHCI-x99-injector.kext|For unsupported USB chipsets on the X99 platform.
...
AppleALC.kext|Onboard Audio support. You need to find out your layout-id (alcid). Add it in the bootargs e.g. 'alcid=1'. To find out your alcid/layout-id, see here: [Supported Codecs](https://github.com/acidanthera/applealc/wiki/supported-codecs)
...
In the Kernel-Emulate section of the config.plist.
...
# Ethernet
Most Ethernet ports should be working with IntelMausi.kext.
